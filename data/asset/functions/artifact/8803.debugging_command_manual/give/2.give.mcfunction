...
# 神器の説明文 (TextComponentString[])
    data modify storage asset:artifact Lore set value ['{"text":"様々なデバッグ用のコマンドについて書かれた本。"}','{"text":"コマンドを実行できる権限が無いと使うことができない。"}','{"translate":"取り扱いには%1$sだ。", "with":[{"text":"注意が必要","color":"red"}]}']
...
# カスタムNBT (NBTCompound) 追加で指定したいNBT (オプション)
    ...
    {"text":"\\n\\n[神器の一括取得]", "color":"dark_purple", "hoverEvent": {"action": "show_text", "value": "debug:catalog_container/artifact/give.m"}, "clickEvent":{"action":"run_command", "value":"/tellraw @s [{\\"color\\":\\"white\\", \\"text\\":\\"\\\\n指定の神器が全て入ったチェストを入手できる。\\\\n重い処理を行うため、取得するまでにラグが発生することがある。\\\\nまた、インベントリに入りきらなかった分は消滅してしまう。\\\\n\\\\\\"all\\\\\\"では全神器を取得するが、\\\\\\"シャード-ランク\\\\\\"で絞り込むことも可能。\\\\n例：\\\\\\"normal-1\\\\\\"、\\\\\\"red-4\\\\\\"、\\\\\\"blue-2\\\\\\"、\\\\\\"green-all\\\\\\"\\\\n\\\\\\"all\\\\\\"の場合はIDが5000を超えるものは取得できないため注意。\\\\n本データパックでは非シャード産の神器のみを指定する\\\\n\\\\\\"exception-all\\\\\\"と\\\\\\"exception-boss\\\\\\"も追加されている。\\"}, {\\"color\\":\\"gold\\", \\"text\\":\\"\\\\n[例文をチャット欄に入力]\\", \\"clickEvent\\":{\\"action\\":\\"suggest_command\\", \\"value\\":\\"/function debug:catalog_container/artifact/give.m {_:\\\\\\"exception-boss\\\\\\"}\\"}}]"}},\
