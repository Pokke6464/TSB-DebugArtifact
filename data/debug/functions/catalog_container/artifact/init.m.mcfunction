...
# IDリストをセット (非シャード産指定の場合)
    execute if data storage debug:catalog_container {Arg:"exception-all"} run function debug:catalog_container/artifact/generate_id_list/exception/all
    execute if data storage debug:catalog_container {Arg:"exception-boss"} run function debug:catalog_container/artifact/generate_id_list/exception/boss
